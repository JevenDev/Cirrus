...
import static com.jvn.cirrus.client.util.CirrusRandom.signedDouble;
...
public final class CirrusLightningRenderer {
    ...
    private static final RenderPipeline CORE_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.MATRICES_FOG_SNIPPET)
    ...
    private static final Map<Long, CachedGeometry> GEOMETRY_CACHE =
            new LinkedHashMap<>(64, 0.75F, true) {
                @Override
                protected boolean removeEldestEntry(Map.Entry<Long, CachedGeometry> eldest) {
                    return size() > 64;
                }
            };
    ...
    public static void initialize() {
        // Forces the custom lightning pipelines to register during client initialization.
    }
    ...
    public static void submit(
        ...
        CachedGeometry geometry = GEOMETRY_CACHE.get(seed);
        if (geometry == null || !geometry.visualOrigin().equals(visualOrigin)) {
            ...
            GEOMETRY_CACHE.put(seed, geometry);
        }
        List<Segment> segments = geometry.segments();

        float configuredIntensity = CirrusConfig.LIGHTNING_BOLT_INTENSITY.get().floatValue();
        float seedPhase = (float)((seed & 1023L) * (Mth.TWO_PI / 1024.0));
        float pulse = 0.84F + 0.16F * Mth.cos(age * 11.0F + seedPhase);
        float intensity = configuredIntensity * pulse;

        collector.submitCustomGeometry(poseStack, GLOW_RENDER_TYPE, (pose, consumer) -> {
            Matrix4f matrix = pose.pose();
            drawLayer(matrix, consumer, segments, 4.8F, 0.22F, 0.32F, 1.00F,
                    0.055F * intensity, false);
            drawLayer(matrix, consumer, segments, 2.15F, 0.48F, 0.68F, 1.00F,
                    0.13F * intensity, false);
        });
        collector.submitCustomGeometry(poseStack, CORE_RENDER_TYPE, (pose, consumer) -> {
            Matrix4f matrix = pose.pose();
            drawLayer(matrix, consumer, segments, 1.0F, 0.88F, 0.94F, 1.00F,
